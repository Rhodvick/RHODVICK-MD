...
try {
    ...
    await client.groupAcceptInvite(result)
        .then(() => m.reply(`Rhodvick has joined ${subject}`))
        .catch((res) => {
            if (res.data == 400) return m.reply('Group does not exist.');
            if (res.data == 401) return m.reply('Rhodvick was previously removed, cannot join using link.');
            if (res.data == 409) return m.reply('Rhodvick was already in the group, Uh?');
