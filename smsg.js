...
function smsg(rhodvickInstance, message, store) {
  if (!message) {
    return message;
  }
  ...
  if (message.key) {
    ...
    message.sender = rhodvickInstance.decodeJid(
      message.fromMe && rhodvickInstance.user.id || message.participant || message.key.participant || message.chat || ''
    );
    
    if (message.isGroup) {
      message.participant = rhodvickInstance.decodeJid(message.key.participant) || '';
    }
  }
  ...
  if (message.message) {
    ...
    if (quotedMessage) {
      ...
      message.quoted.sender = rhodvickInstance.decodeJid(message.msg.contextInfo.participant);
      message.quoted.fromMe = message.quoted.sender === rhodvickInstance.decodeJid(rhodvickInstance.user.id);
      ...
      message.quoted.getQuotedObj = message.quoted.getQuotedMessage = async () => {
        ...
        let quotedMsg = await store.loadMessage(message.chat, message.quoted.id, rhodvickInstance);
        return exports.smsg(rhodvickInstance, quotedMsg, store);
      };
      ...
      message.quoted["delete"] = () => rhodvickInstance.sendMessage(message.quoted.chat, { 'delete': quotedMessageFakeObj.key });
      message.quoted.copyNForward = (toChat, forceForward = false, options = {}) => rhodvickInstance.copyNForward(toChat, quotedMessageFakeObj, forceForward, options);
      message.quoted.download = () => rhodvickInstance.downloadMediaMessage(message.quoted);
    }
  }

  // Handle URL in the message
  if (message.msg.url) {
    message.download = () => rhodvickInstance.downloadMediaMessage(message.msg);
  }
  ...
  message.reply = (replyText, replyTo = message.chat, options = {}) => {
    if (Buffer.isBuffer(replyText)) {
      return rhodvickInstance.sendMedia(replyTo, replyText, "file", '', message, { ...options });
    }
    return rhodvickInstance.sendText(replyTo, replyText, message, { ...options });
  };

  // Function to copy the message
  message.copy = () => exports.smsg(rhodvickInstance, messageInfo.fromObject(messageInfo.toObject(message)));

  // Function to forward the message
  message.copyNForward = (toChat = message.chat, forceForward = false, options = {}) => rhodvickInstance.copyNForward(toChat, message, forceForward, options);
