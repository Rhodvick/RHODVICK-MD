...
const daddy = "254700150919@s.whatsapp.net";
...
module.exports = Rhodvick = async (client, m, chatUpdate, message, store) => {
  try {
    var body =
    ...
    var msgRhodvick = m.message.extendedTextMessage?.contextInfo?.quotedMessage;

    var budy = typeof m.text == "string" ? m.text : "";
   
    const timestamp = speed(); 
    const Rhodvickspeed = speed() - timestamp;
    ...
    const DevRhodvick = dev.split(",");
    const Owner = DevRhodvick.map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
    ...
    const context = {
        ...
        pushname, body, budy, totalCommands, args, mime, qmsg, msgRhodvick, botNumber, itsMe,
        packname, author, generateProfilePicture, groupMetadata, Rhodvickspeed, mycode,
